...
class IdentityTransform:
    """Identity transform that can be pickled (unlike lambda functions)."""
    def __call__(self, x):
        return x

class FaceImagesDataset(Dataset):
    def __init__(self, path_to_data="/home-local/rudravg/projects/DisCo_latents/face_images", transform=None):
        # Get all .npy files from the directory
        self.image_files = glob.glob(f"{path_to_data}/*.npy")
    ...
    def __getitem__(self, idx):
        ...
        # Return tuple (data, label) as expected by training code
        # Since there are no labels for face images, we use 0 as dummy label
        return image, 0

def get_face_dataloader(batch_size=128,
                       path_to_data="./face_images",
                       transform=None,
                       shuffle=True,
                       num_workers=4):
    """
    FaceImages dataloader — returns a DataLoader that yields
    batches of shape (N, 1, H, W), normalized to [0,1].
    ...
    path_to_data : str
        Directory containing your .npy face images.
    transform : callable or None
        Optional torchvision‐style transform applied after loading
        (e.g. data augmentation). By default no extra transform.
    shuffle : bool
        Whether to shuffle the dataset each epoch.
    num_workers : int
        Number of worker processes for loading data.
    """
    # if user didn't pass a transform, just do identity
    if transform is None:
        transform = IdentityTransform()

    face_ds = FaceImagesDataset(path_to_data=path_to_data,
                                transform=transform)
    face_loader = DataLoader(face_ds,
